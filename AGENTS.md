...
- Before a multi-file `apply_patch`, verify every context anchor against the live files; when one anchor differs, split the edit into per-file patches using the verified anchors so one stale context cannot reject the entire repair.
