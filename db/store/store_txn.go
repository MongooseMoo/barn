...
func (tx *StoreTxn) HasWrites() bool {
	return tx != nil && (len(tx.scalarWrites) > 0 || len(tx.propertyDefines) > 0 || len(tx.propertyWrites) > 0 || len(tx.propertyDeletes) > 0)
}
...
func (tx *StoreTxn) Commit() types.ErrorCode {
	if tx == nil || (len(tx.scalarWrites) == 0 && len(tx.propertyDefines) == 0 && len(tx.propertyWrites) == 0 && len(tx.propertyDeletes) == 0) {
		return types.E_NONE
	}
	if tx.store == nil {
		return types.E_INVARG
...
func (tx *StoreTxn) findVerb(objID types.ObjID, verbName string) (*Verb, types.ObjID, error) {
	return tx.findVerbWalk(objID, verbName, false)
}

// FindCallableVerb mirrors Store.FindCallableVerb on the transaction snapshot:
// it walks the ancestry chain exactly like FindVerb, but a same-named verb that
// lacks the execute ("x") permission does not shadow same-named executable verbs
// on more distant ancestors — the walk treats it as a non-match and keeps
// searching further up the chain (ToastStunt obj:verb() dispatch semantics).
func (tx *StoreTxn) FindCallableVerb(objID types.ObjID, verbName string) (VerbView, types.ObjID, error) {
	verb, definer, err := tx.findVerbWalk(objID, verbName, true)
	if err != nil {
		return VerbView{}, definer, err
	}
	return verb.View(), definer, nil
...
func (tx *StoreTxn) findVerbWalk(objID types.ObjID, verbName string, requireExecute bool) (*Verb, types.ObjID, error) {
	visited := make(map[types.ObjID]bool)
	queue := []types.ObjID{objID}
