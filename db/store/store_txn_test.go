...
func TestReadOnlyTransactionLoadsObjectsLazily(t *testing.T) {
	...
	if got := len(tx.objects); got != 0 {
		t.Fatalf("BeginSnapshot cached %d objects, want 0", got)
	}
...
func TestTransactionDisjointPropertyWritesBothCommit(t *testing.T) {
	...
	txA := store.BeginSnapshot(0)
	txB := store.BeginSnapshot(0)
...
func TestTransactionSamePropertyWriteConflicts(t *testing.T) {
	...
	first := store.BeginSnapshot(0)
	second := store.BeginSnapshot(0)
...
func TestTransactionPropertyInfoConflictsWithValueWrite(t *testing.T) {
	...
	infoTx := store.BeginSnapshot(0)
	valueTx := store.BeginSnapshot(0)
	newPerms := PropRead
...
func TestTransactionPropertyDefinitionDecentralizedCommitIgnoresCachedMissingObjects(t *testing.T) {
	store := NewStore()
	if err := store.Add(NewObject(0, 0)); err != nil {
		t.Fatalf("Add root failed: %v", err)
	}
	...
	tx := store.BeginSnapshot(0)
	defer tx.Release()
	if errCode := tx.ObjectExists(999); errCode != types.E_INVIND {
		t.Fatalf("ObjectExists missing object = %v, want E_INVIND", errCode)
	}
	if errCode := tx.DefineProperty(0, "kept", NewProperty(types.NewInt(1), 0, PropRead|PropWrite, false, true)); errCode != types.E_NONE {
		t.Fatalf("DefineProperty failed: %v", errCode)
	}
...
func TestTransactionPropertyDefinitionCoarseCommitIgnoresCachedMissingObjects(t *testing.T) {
	store := NewStore()
	if err := store.Add(NewObject(0, 0)); err != nil {
		t.Fatalf("Add root failed: %v", err)
	}
...
func TestTransactionScalarAndPropertyWritesSameObjectBothCommit(t *testing.T) {
	...
	scalarTx := store.BeginSnapshot(0)
	propertyTx := store.BeginSnapshot(0)
...
func TestTransactionLiveMutationDoesNotRebaseUnrelatedReads(t *testing.T) {
	...
	concurrent := store.BeginSnapshot(0)
