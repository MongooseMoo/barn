...
func TestSchedulersOwnIsolatedTaskManagers(t *testing.T) {
	first := NewScheduler(dbstore.NewStore())
	second := NewScheduler(dbstore.NewStore())
	t.Cleanup(first.Stop)
	t.Cleanup(second.Stop)

	if first.taskManager == second.taskManager {
		t.Fatal("two schedulers share one task manager")
	}
	...
	assertQueuedTask := func(t *testing.T, scheduler *Scheduler, wantID int64) {
		t.Helper()
		...
		result, ok := scheduler.registry.CallByName("queued_tasks", ctx, nil)
		if !ok {
