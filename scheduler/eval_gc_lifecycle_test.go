...
func TestEvalRunGCCallbackCreatedAnonymousSurvivesNextEvalYield(t *testing.T) {
	...
	runEvalTask := func(code string) *task.Task {
		program := compileTestProgram(t, scheduler.registry, code)
		...
		for pass := 0; pass < 8; pass++ {
			...
			if processed := scheduler.ProcessReadyTasks(); processed == 0 {
...
func TestRunGCValidationConflictDoesNotRecycleNewPersistentRoot(t *testing.T) {
	...
	t.Cleanup(func() { removeTasksForOwner(scheduler, 0) })
	scheduler.registry.Register("commit_subject_b", func(_ *kernel.TaskContext, args []types.Value) types.Result {
		if len(args) != 0 {
...
func TestRunGCCommitsStagedAnonymousEdgeBeforeLiveSweep(t *testing.T) {
	...
	t.Cleanup(scheduler.Stop)
	t.Cleanup(func() { removeTasksForOwner(scheduler, 0) })
	scheduler.registry.Register("stage_cycle_edge", func(ctx *kernel.TaskContext, args []types.Value) types.Result {
	...
	program := compileTestProgram(t, scheduler.registry, "stage_cycle_edge(); run_gc(); return 1;")
	taskID := scheduler.CreateBackgroundTask(0, program, 0)
	running := scheduler.GetTask(taskID)
	running.Context.IsWizard = true
	if err := scheduler.runTask(running); err != nil {
