...
func buildLiveTask(s *Scheduler, id int64, wizardID types.ObjID, prog *bytecode.Program) *task.Task {
...
func runMongooseWorkload(t *testing.T, fx *mongooseFixture, active int, mix workloadMix, warmup, measure time.Duration) workloadResult {
	...
	s := newSchedulerWithWorkerCount(fx.store, config.Options{}, runtime.GOMAXPROCS(0))
	defer s.Stop()
