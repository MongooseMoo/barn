...
func TestDeferredGCSweepBlocksNewVMStartUntilSweepCompletes(t *testing.T) {
	...
	scheduler.registry.Register("recycle", func(ctx *kernel.TaskContext, args []types.Value) types.Result {
		sweepOnce.Do(func() {
			nestedSweepVerb <- scheduler.registry.CallVerb(0, "sweep_probe", nil, ctx)
	...
	scheduler.registry.Register("gc_task_started", func(_ *kernel.TaskContext, args []types.Value) types.Result {
		if len(args) != 1 || args[0].Type() != types.TYPE_ANON {
			return types.Err(types.E_INVARG)
		}
	...
	gcCtx.IsWizard = true
	gcCtx.Store = store
	gcCtx.Registry = scheduler.registry
	...
	scheduler.deferAnonGC(gcCtx, firstCandidate, nil)
	scheduler.deferAnonGC(gcCtx, firstCandidate, nil)
	...
	program := compileTestProgram(t, scheduler.registry, "held = create(#0, 1); gc_task_started(held); suspend(); return held;")
	taskID := scheduler.CreateBackgroundTask(0, program, 0)
	...
	go func() {
		processed <- scheduler.ProcessReadyTasks()
	}()
	select {
	case <-startAttempted:
	case <-time.After(5 * time.Second):
...
func TestDeferredGCSweepBlocksEvalVMStart(t *testing.T) {
	...
	release := func() { releaseOnce.Do(func() { close(releaseSweep) }) }
	defer release()
	scheduler.registry.Register("recycle", func(_ *kernel.TaskContext, args []types.Value) types.Result {
		close(sweepEntered)
		<-releaseSweep
		if err := store.Recycle(args[0].Obj()); err != nil {
			return types.Err(types.E_INVARG)
	...
	gcCtx.Store = store
	gcCtx.Registry = scheduler.registry
	scheduler.deferAnonGC(gcCtx, candidate, nil)
	sweepDone := make(chan struct{})
	go func() {
		scheduler.flushDeferredGC()
		close(sweepDone)
	...
	scheduler.registry.Register("gc_eval_started", func(_ *kernel.TaskContext, _ []types.Value) types.Result {
		close(evalEntered)
		return types.Ok(types.NewInt(0))
	})
	startAttempted := make(chan struct{})
...
func TestDeferredGCSweepBlocksServerHookVMStart(t *testing.T) {
	...
	scheduler.registry.Register("gc_server_hook_started", func(_ *kernel.TaskContext, _ []types.Value) types.Result {
		close(hookEntered)
		return types.Ok(types.NewInt(0))
	})
	startAttempted := make(chan struct{})
	...
	go func() { hookDone <- scheduler.CallVerb(0, "server_probe", nil, 0) }()
	select {
	case <-startAttempted:
	case <-time.After(5 * time.Second):
