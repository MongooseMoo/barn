...
func TestShutdownCannotPublishBetweenReadySelectionAndDispatch(t *testing.T) {
	resetServerVerbTaskManager(t)
	t.Cleanup(func() { resetServerVerbTaskManager(t) })
	store := dbstore.NewStore()
	addServerVerbTestObject(t, store, 0, dbstore.FlagWizard)
	...
	tk := task.NewTaskFull(94001, 0, compileTestProgram(t, s.registry, `a = create(#0, #0, 1);`), 1<<50, 1e9)
	tk.Context.IsWizard = true
	if got := s.QueueTask(tk); got != tk.ID {
		t.Fatalf("QueueTask = %d, want %d", got, tk.ID)
	}

	selected := make(chan struct{})
	release := make(chan struct{})
	s.taskLifecycleObserver = func(stage string, _ *task.Task) {
		if stage == "ready_after_reserve" {
			close(selected)
			<-release
		}
	}
	passDone := make(chan int, 1)
	go func() { passDone <- s.ProcessReadyTasks() }()
	select {
	case <-selected:
	case <-time.After(time.Second):
		t.Fatal("ready task was not selected")
	}

	ready := s.BeginShutdown(nil)
	select {
	case <-ready:
		t.Fatal("shutdown published between ready selection and worker dispatch")
	default:
	}
	close(release)
	if got := <-passDone; got != 1 {
		t.Fatalf("ProcessReadyTasks = %d, want 1", got)
	}
	select {
	case <-ready:
	case <-time.After(time.Second):
		t.Fatal("shutdown did not publish after reserved ready pass settled")
	}
	if got := len(store.Snapshot().PendingFinalizations); got != 1 {
		t.Fatalf("pending roots = %d, want selected task root", got)
	}
}

func TestShutdownCannotPublishBetweenImmediateRegistrationAndExecution(t *testing.T) {
	resetServerVerbTaskManager(t)
	t.Cleanup(func() { resetServerVerbTaskManager(t) })
	store := dbstore.NewStore()
	addServerVerbTestObject(t, store, 0, dbstore.FlagWizard)
	store.AddVerb(0, dbstore.NewVerb("immediate_root", []string{"immediate_root"}, 0,
		dbstore.VerbRead|dbstore.VerbExecute,
		dbstore.VerbArgs{This: "this", Prep: "none", That: "this"},
		[]string{`a = create(#0, #0, 1);`}))
	s := NewScheduler(store)
	t.Cleanup(s.Stop)
	s.SetPendingFinalizationSink(store.AppendPendingFinalizations)
	...
	release := make(chan struct{})
	s.taskLifecycleObserver = func(stage string, _ *task.Task) {
		if stage == "immediate_after_register" {
			close(registered)
			<-release
		}
	}
	runDone := make(chan error, 1)
	go func() {
		_, err := s.RunServerVerbTask(0, "immediate_root", nil, 0)
		runDone <- err
	}()
	select {
	case <-registered:
	case <-time.After(time.Second):
		t.Fatal("immediate task was not registered")
	}

	ready := s.BeginShutdown(nil)
	select {
	case <-ready:
		t.Fatal("shutdown published between immediate registration and execution")
	default:
	}
	close(release)
	if err := <-runDone; err != nil {
		t.Fatalf("RunServerVerbTask: %v", err)
	}
	select {
	case <-ready:
	case <-time.After(time.Second):
		t.Fatal("shutdown did not publish after immediate task settled")
	}
	if got := len(store.Snapshot().PendingFinalizations); got != 1 {
		t.Fatalf("pending roots = %d, want immediate task root", got)
	}
}
