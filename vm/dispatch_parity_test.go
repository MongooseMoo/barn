...
var dispatchParityCorpus = []struct {
	...
	{"for_in_list_indexed", "s = 0; for x, k in ({10, 20, 30}); s = s + x + k; endfor; return s;"},
	{"for_in_map", `s = {}; for x in (["b" -> {2}, "a" -> {1}]); s = {@s, x}; endfor; return s;`},
	{"for_in_map_indexed", `s = {}; for x, k in (["b" -> 2, "a" -> 1]); s = {@s, {x, k}}; endfor; return s;`},
	{"for_in_duplicate_names", `s = {}; for x, x in ({10, 20, 30}); s = {@s, x}; endfor; return s;`},
	{"for_in_unicode_indexed", `s = {}; for x, k in ("a界😀"); s = {@s, {x, k}}; endfor; return s;`},
