...
func builtinArgon2(ctx *runtime.TaskContext, args []types.Value) types.Result {
	if !ctx.IsWizard {
...
func builtinArgon2Verify(ctx *runtime.TaskContext, args []types.Value) types.Result {
	if !ctx.IsWizard {
		return types.Err(types.E_PERM)
	}
	if len(args) != 2 {
