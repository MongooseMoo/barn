...
func TestSwitchPlayerReturnsNoValueOnSuccess(t *testing.T) {
	...
	// switch_player now requires the old player to have a resolvable
	// connection (it switches that connection's player), so the stub must
	// supply one. Without a StoreTxn the switch runs immediately.
