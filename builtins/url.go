...
func builtinUrlEncode(ctx *runtime.TaskContext, args []types.Value) types.Result {
...
func builtinUrlDecode(ctx *runtime.TaskContext, args []types.Value) types.Result {
	if len(args) != 1 {
		return types.Err(types.E_ARGS)
	}
	s, ok := args[0].(types.StrValue)
