...
// findCallableVerbForRead resolves a verb the way obj:verb() call dispatch does
// (skipping a same-named non-executable verb that would otherwise shadow an
// executable one further up the ancestry chain), routed through the task's read
// transaction. Use this only for call-semantics queries such as respond_to();
// verb management/introspection builtins must use findVerbForRead instead so
// they can find verbs regardless of the execute permission.
