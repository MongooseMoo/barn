...
type BuiltinFunc func(ctx *runtime.TaskContext, args []types.Value) types.Result
...
type VerbCallerFunc func(objID types.ObjID, verbName string, args []types.Value, ctx *runtime.TaskContext) types.Result
...
func (r *Registry) Register(name string, fn BuiltinFunc) {
	if _, ok := lookupFunctionSignature(name); ok {
		inner := fn
		fn = func(ctx *runtime.TaskContext, args []types.Value) types.Result {
...
// redirection first.
func (r *Registry) CallByID(id int, ctx *runtime.TaskContext, args []types.Value) types.Result {
	fn, ok := r.byID[id]
	if !ok {
...
// redirection first.
func (r *Registry) CallByName(name string, ctx *runtime.TaskContext, args []types.Value) (types.Result, bool) {
	fn, ok := r.funcs[name]
	if !ok {
...
func (r *Registry) dispatch(name string, fn BuiltinFunc, ctx *runtime.TaskContext, args []types.Value) types.Result {
...
func (r *Registry) maybeProtectedRedirect(name string, ctx *runtime.TaskContext, args []types.Value) (types.Result, bool) {
	if ctx == nil || name == "" {
		return types.Result{}, false
	}
...
func (r *Registry) CallVerb(objID types.ObjID, verbName string, args []types.Value, ctx *runtime.TaskContext) types.Result {
	if r.verbCaller == nil {
		return types.Err(types.E_VERBNF)
	}
