...
func main() {
	...
	parents, _ := store.DirectTxn().Parents(2)
	...
	if v, ok, _ := store.DirectTxn().LocalProperty(2, "password"); ok {
	...
	if names, errCode := store.DirectTxn().DefinedPropertyNames(2); errCode == types.E_NONE {
		for _, name := range names {
			if v, ok, _ := store.DirectTxn().LocalProperty(2, name); ok {
...
func findPassword(store *dbstore.Store, objID types.ObjID, visited map[types.ObjID]bool, depth int) {
	...
	if v, ok, _ := store.DirectTxn().LocalProperty(objID, "password"); ok {
