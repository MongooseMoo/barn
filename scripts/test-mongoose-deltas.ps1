...
if ($Engine -eq 'Barn') {
    $suiteArgs += @("--candidate-root=$repo", "--moo-suite-root=$repo/tests/mongoose-conformance")
...
$suiteArgs += @("--candidate-root=$($linuxRepo.Trim())", "--moo-suite-root=$($linuxRepo.Trim())/tests/mongoose-conformance", "--server-db=$($linuxRepo.Trim())/Test_conf.db")
